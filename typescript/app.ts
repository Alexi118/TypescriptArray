...
function printArray(data:any){
    var _tr:any =``; 
    for(let obj of data){
        var avgMark:any = ((obj.toan + obj.ly + obj.hoa)/3).toFixed(1);
        let ranking:string = '';
        if(avgMark < 5){
            ranking = 'Yeu';
...
function sortByScore(){
    var sorting : any= document.getElementById('sortOption');
    if (sorting.value == 'asc'){
        let myData = setData();
        let sortData = myData.sort(function(a, b){
            let avgMarkA:any = ((a.toan + a.ly + a.hoa)/3)
            let avgMarkB:any = ((b.toan + b.ly + b.hoa)/3)
